...
# reshape data
todays_covid_cast <- reshape2::dcast(todays_covid_summary, Site + LOC_NAME + DEPARTMENT_NAME + ICU + CensusDate ~
...
# Rename deactivated units
todays_census_covid_merge <- todays_census_covid_merge %>%  mutate(DEPARTMENT_NAME = ifelse(is.na(str_extract(DEPARTMENT_NAME,
                                                                         "(?<=X_).+(?=_DEACTIVATED)")), DEPARTMENT_NAME,
                                                                  str_extract(DEPARTMENT_NAME,  "(?<=X_).+(?=_DEACTIVATED)")))


# Add new variables
todays_census_covid_merge <- todays_census_covid_merge %>% mutate(`non-COVID19` = OCCUPIED_COUNT - `COVID19` - SUSC - PUI - PUM,
...
# Remove data sets
rm(todays_covid_summary, todays_covid_data, todays_covid_cast, todays_census_covid_merge )
rm(todays_census_subset, todays_census_data, unit_mapping, site_mapping, repo)
...
raw_data <- raw_data %>% group_by(CensusDate, Site, `Unit Type High`) %>% mutate(count=ifelse(Site=="MSSN", n(), 1),
                                                                                 check=ifelse(Site=="MSSN", 1: n(), 1))
table(raw_data$count)
table(raw_data$check)


raw_data <- raw_data[!(raw_data$count==2 &  raw_data$check==1),]

raw_data <- subset(raw_data, select= -c(count, check)) 
raw_data <- raw_data %>% ungroup()


# Save the new repo
start_date <- min(raw_data$CensusDate)
end_date <- max(raw_data$CensusDate)

write_xlsx(raw_data, path = paste0(Census_path, "\\REPO\\Census and Covid Repo ", 
                                   start_date, " to ", end_date, " Created ", Sys.Date(), " Add MSSN.xlsx"))
